  ...
  fluidRow(
    ...
    absolutePanel(id = "controls", class = "panel panel-default", fixed = TRUE,
                  ...
                  selectInput("data_type", "", 
                              c("Observado", "Anomalía absoluta", "Anomalía relativa")),
                  actionButton("hide", "",
...
server <- function(input, output, session) {
  ...
  data_sel  <- reactive({
    if(input$data_type == "Anomalía absoluta"){
     r <- "./data/horas_sol_esp_anom.tiff"
     return(r)
    }
    
    if(input$data_type == "Observado"){
      
    r <- "./data/horas_sol_esp.tiff"
      return(r)
    }
    
    if(input$data_type == "Anomalía relativa"){
      
    r <- "./data/horas_sol_esp_anomrel.tiff"
...
# color palette by data input 
  colpal  <- reactive({
    ...
    if(input$data_type == "Anomalía relativa"){
      ...
      cb <- colorBin(palette = col3, bins = cutp*100, na.color = "transparent")
      tmp <- list(cb = cb, lab =  lab_temp, col = col3, cut = cutp * 100, colcut = col3,
    ...
    if(input$data_type == "Observado"){
      ...
      at[15] <- 450
      at <- c(0, at)
      cb <- colorBin(palette = col, bins = at, na.color = "transparent")
      lab_temp <- scales::number(parse_number(rev(leg$lab)), suffix = "h")
      lab_temp[c(1, 15)] <- c("> 361h", "< 117h")
      tmp <- list(cb = cb, lab =   lab_temp, col = rev(col), colcut = col, title = "Horas de sol", cut = at)
      return(tmp)
    }
    ...
    if(input$data_type == "Anomalía absoluta"){
      ...
      cb <- colorBin(palette = col3, bins = cutp, na.color = "transparent")
      tmp <- list(cb = cb, lab =  lab_temp, col = col3, cut = cutp, colcut = col3,
                  title = "Anomalía")
      return(tmp)
    }
...
output$ggi <- renderGirafe({
  ...
  r <- rast("./data/horas_sol_esp_WGS84.tiff") 
...
observeEvent(input$hide != 0, {
  ...
  observe({
    ...
    ts <- as_date(as_datetime(time(rast(data_sel()))))
  ...
  tic()
    leafletProxy("map", session) %>%
      ...
      clearControls() %>%
      addGeotiff(data_sel(), 
                 colorOptions = colorOptions(palette = colpal()$colcut,
                                             breaks = colpal()$cut,
                                             na.color = "transparent"),
                 tileOptions = tileOptions(tileSize = 5),
                 bands = dy,
                 opacity = 0.8,
                 project = T,
                 autozoom = FALSE,
